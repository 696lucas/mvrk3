...
function getCartState() {
  try {
    return JSON.parse(sessionStorage.getItem("pb_cart") || "{}");
  } catch {
    return {};
  }
}
function setCartState(s) {
  try {
    sessionStorage.setItem("pb_cart", JSON.stringify(s));
  } catch {}
}
...
export default function CartDrawer() {
  ...
  useEffect(() => {
    ...
    ov.textContent = `
      ...
      .pb-cart-dlg__panel{ background:#fff !important; color:#111 !important; width:min(86vw, 320px) !important; box-shadow:-10px 0 28px rgba(0,0,0,.35) !important;  z-index:2; }
    `;
  ...
  useEffect(() => {
    ...
    window.PB_goToCheckout = async () => {
      const id = await ensureCartId();
      if (!id) return;
      const d = await gql(CART_QUERY, { id });
    ...
    const triggers = Array.from(
      document.querySelectorAll(".pb-cart-trigger, a[href=\"/cart\"]")
    );
    const onClick = (e) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  // Delegación: cualquier [data-close] dentro del diálogo cierra el modal
  useEffect(() => {
  ...
  return (
    <div id="pb-cart-dlg" className={open ? "is-open" : ""}>
      <div
        className="pb-cart-dlg__overlay"
        data-close
        onClick={closeCartPopup}
      />
      <aside
        className="pb-cart-dlg__panel"
        role="dialog"
        aria-modal="true"
        aria-label="Carrito"
      >
        <div className="pb-cart__head">
          <div className="pb-cart__title">
            Carrito{" "}
            <span
              id="pb-cart-count"
        ...
        <div id="pb-cart-list" className="pb-cart__list">
          {(cart?.lines?.edges || []).map(({ node }) => {
            ...
            return (
              <div
                className="pb-cart__item"
                key={node.id}
                data-line-id={node.id}
              >
                <div className="pb-cart__thumb">
                  <Image
                    width={39}
                    height={39}
                    src={img}
                ...
                <div>
                  ...
                  <div className="pb-cart__qty">
                    <button
                      className="pb-cart__btn"
                      onClick={() =>
                        cartUpdate(node.id, Math.max(1, node.quantity - 1))
                      }
                    >
                      –
                    </button>
                    ...
                    <button
                      className="pb-cart__btn"
                      onClick={() =>
                        cartUpdate(node.id, node.quantity + 1)
                      }
                    >
                      +
                    </button>
                    <span
                      className="pb-cart__trash"
                      title="Eliminar"
                      onClick={() => cartRemove(node.id)}
                    >
                      🗑️
                    </span>
                  </div>
                </div>
                <div className="pb-cart__price">
                  {formatPrice(+priceAmt, priceCur)}
                </div>
              </div>
            );
          })}
        ...
        <div className="pb-cart__foot">
          <div className="pb-cart__row">
            ...
            <div id="pb-cart-total" style={{ fontWeight: 800 }}>
              {formatPrice(
                Number(cart?.cost?.subtotalAmount?.amount || 0),
                cart?.cost?.subtotalAmount?.currencyCode || "EUR"
              )}
            </div>
          </div>
          <button
